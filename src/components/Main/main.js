...
function Main(props) {
    return (
        ...
        <p className='views'><img className="views--icon" src={viewsIcon} alt="views--icon"/>{props.views}</p>
        </div>
        <div className='mobile--row--two'>
        <p className='date'>{props.timestamp}</p>
        <p className='likes'><img className="likes--icon" src={likesIcon} alt="likes--icon"/>{props.likes}</p>
        </div>
        ...
        <form action="" className="comments--form">
            <label htmlFor="comment" className="comments--label">JOIN THE CONVERSATION</label>
            <input type="text" id='user--comment' placeholder='Add a new comment' />
